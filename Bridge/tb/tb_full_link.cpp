// ============================================================================
// 本文件是 axi2flit/systemc/tb/tb_full_link.cpp 的副本，用于验证 Bridge。
//
// 上游仓库保持只读；这里只改了两处后端接线，其余（五个 AXI 通道的激励、
// 独立 scoreboard、12 个用例、终局断言）完全沿用，以便直接用那套已经验证过
// 的测试来衡量新写的 Bridge。
//
// 默认验证 C++ AouTarget + MemSimBackend；定义 RTL_BRIDGE 后，以同一套激励和
// scoreboard 验证 Verilated storage_bridge_top + 在线 mem_sim。
//
// 上游来源：axi2flit @ c169ee8
// ============================================================================
/**
...
private:
    ...
    void monitor() {
        ...
        while (true) {
            ...
            if (rv.read() && rr.read()) {
                ...
                if (got.resp != 0 && got.last) ++r_error_count;
    ...
    void stimulus() {
        try {
...
#ifdef RTL_BRIDGE
            check(target.completed == target.submitted + target.synthesized_errors,
                  "RTL child response accounting mismatch");
#else
...
#ifdef RTL_BRIDGE
            std::cout << "RTL_BRIDGE: submitted=" << target.submitted
                      << " completed=" << target.completed
                      << " retries=" << target.retries
                      << " synthesized_errors=" << target.synthesized_errors << "\n";
            std::cout << "DIFF: mc_submitted=" << target.submitted
                      << " parent_errors=" << b_error_count + r_error_count
                      << " AW=" << aw_count << " W=" << w_count
                      << " B=" << b_count << " AR=" << ar_count
                      << " R=" << r_count << "\n";
#else
            std::cout
            ...
            std::cout << "DIFF: mc_submitted=" << memory.submitted_txns
                      << " parent_errors=" << b_error_count + r_error_count
                      << " AW=" << aw_count << " W=" << w_count
                      << " B=" << b_count << " AR=" << ar_count
                      << " R=" << r_count << "\n";
#endif
            std::cout << "LINK: forward=" << stats.forward.tx_new_flits << " reverse=" << stats.reverse.tx_new_flits
            ...
            check(false, e.what());
#ifdef RTL_BRIDGE
            std::cerr << "RTL timeout state: submitted=" << target.submitted
                      << " completed=" << target.completed
                      << " retries=" << target.retries
                      << " synthesized_errors=" << target.synthesized_errors
                      << " tx(R/B/C)=" << target.tx_read_responses << '/'
                      << target.tx_write_responses << '/' << target.tx_credit_only
                      << " grants(WQ/RQ/WD)=" << target.granted_wreq << '/'
                      << target.granted_rreq << '/' << target.granted_wdata
                      << " AW/W/B/AR/R=" << aw_count << '/' << w_count << '/'
                      << b_count << '/' << ar_count << '/' << r_count << "\n"
                      << "RTL internals: " << target.debug_state() << "\n";
#endif
            csv.flush(); done = true; sc_stop();
        }
    }
};
...
#ifdef RTL_BRIDGE
    if (rp != 1) {
        std::cerr << "RTL Bridge P6 currently supports --rp 1 only\n";
        return 2;
    }
#endif
